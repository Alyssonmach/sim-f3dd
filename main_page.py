...
def remove_accents(input_str: str):
    ...
    Args:
        input_str (str): reference string.
...
def page1():
    ...
    # Loading the application's adjustable parameters
    ...
    if 'nodes' not in st.session_state or 'conexions' not in st.session_state or 'layers_dict' not in st.session_state:
        ...
        st.session_state['plotter'] = plot_by_layers_streamlit(nodes_list = nodes, conexions_list = conexions, layers_dict = layers_dict, visualize_IDs = True)
    else:
    ...
    with st.expander('Configuração da Simulação', expanded = True):
        ...
        loaded_nodes = st.multiselect('Nós Carregados:', [node[0] for node in nodes], default = [186, 195, 7, 61] ,help = 'Nós que receberão carga')
